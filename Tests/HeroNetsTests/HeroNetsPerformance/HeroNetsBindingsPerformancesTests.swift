...
//    factory: MFDDFactory<KeyMFDD<Var>, Val>
//  )
...
//      for i in 0 ..< len {
//        for _ in 0 ..< 2 {
//          seq.insert(Val(stringLiteral: String(i)))
//        }
//      }
//      print(seq)
...
//      interpreter: interpreter
//    )
//    let factory = MFDDFactory<KeyMFDD<Var>,ValueMFDD>()
//    // How many values in the place
//    var res: [Int: (avgTime: Double, stdTime: Double, stateNumber: Int)] = [:]
//    var seq: Multiset<Val>  = []
//    var marking: Marking<P1>
...
//    var interpreter = Interpreter()
//    try! interpreter.loadModule(fromString: module)
//    let conditionList: [Guard]? = [Pair(x, .val("2"))]
//    let model = HeroNet<P1, T1>(
//      .pre(from: .p1, to: .apply, labeled: [x,y]),
//      .pre(from: .p2, to: .apply, labeled: [f]),
//      .post(from: .apply, to: .res, labeled: [.exp("$f($x,$y)")]),
...
//    var interpreter = Interpreter()
//    try! interpreter.loadModule(fromString: module)
//    let conditionList: [Guard]? = [Pair(x, .exp("$y+1"))]
//    let model = HeroNet<P1, T1>(
//      .pre(from: .p1, to: .apply, labeled: [x,y]),
//      .pre(from: .p2, to: .apply, labeled: [f]),
//      .post(from: .apply, to: .res, labeled: [.exp("$f($x,$y)")]),
...
//    var interpreter = Interpreter()
//    try! interpreter.loadModule(fromString: module)
//    let conditionList: [Guard]? = [Pair(.exp("add($a,5)"), .val("6")), Pair(.exp("eq($c,0)"), .val("false")),Pair(.exp("mod($g($b,$c),2)"), .val("0"))]
//    let model = HeroNet<P1, T1>(
//      .pre(from: .op, to: .apply, labeled: [f,g]),
//      .pre(from: .n, to: .apply, labeled: [a,b, c, d]),
//      .post(from: .apply, to: .res, labeled: [.exp("$f($a,$b)")]),
...
//      .pre(from: .p3, to: .t1, labeled: [b, .val("1")]),
