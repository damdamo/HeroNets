...
final class BaselineNonOptimizedTests: XCTestCase {
  ...
  func simplifyBinding(bindings: Set<[String: Val]>) -> Set<[String:String]> {
    var res: Set<[String:String]> = []
    var dicTemp: [String:String] = [:]
    for el in bindings {
      for (k,v) in el {
        dicTemp[k] = v.description
      }
      res.insert(dicTemp)
      dicTemp = [:]
    }
  ...
  func testBinding0() {
    ...
    let res = baseline.bindingBruteForce(transition: .t1, marking: marking)
  ...
  func testBinding1() {
    ...
    let expectedRes = Set([["$a": "1", "$b": "1", "$x": "2", "$y": "3", "$z": "3"], ["$b": "1", "$a": "1", "$x": "1", "$y": "2", "$z": "2"]])

    XCTAssertEqual(simplifyBinding(bindings: bindings), expectedRes)
  }
